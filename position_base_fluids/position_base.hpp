...
public:
	PositionBase(std::vector<Object*>& objects, const std::size_t transparent_flag, const Bound3D& bound = Bound3D(
		...
		const float& delta_t = 0.1, const int& iter_count = 3);
	void setVelocitys(const std::size_t& i, const Eigen::Vector3f& speed);
	void update();

private:
	// 把object*转换为Sphere
	Sphere& objectToSphere(const std::size_t& i);
	// 获取相邻点
	...
	// 使用viscosity光滑核计算w
	float wViscosity(const Eigen::Vector3f& r);
	// 计算物体i的密度
...
private:
	...
	std::vector<Eigen::Vector3f> velocitys_;                  // 各个物体的速度
};
